...
if model_files:
    ...
    st.sidebar.warning(
        "ยังไม่พบไฟล์โมเดล .pkcls กรุณาวางไฟล์ในโฟลเดอร์ 'models/' "
        "หรืออัปโหลดไฟล์ผ่านช่องด้านบน"
    )

# ============================================================
# 2.1) เลือกตัวสกัด embedding (ต้องตรงกับตอนฝึกโมเดลใน Orange)
# ============================================================
st.sidebar.header("🧬 ตัวสกัดคุณลักษณะภาพ (Embedder)")
embedder_name = st.sidebar.selectbox(
    "เลือก Embedder ให้ตรงกับตอนฝึกโมเดล",
    ["inception-v3", "painters", "vgg16", "vgg19", "squeezenet", "openface", "deeploc"],
    index=0,
    help="โมเดลที่ตรวจพบมี feature 2048 ตัว (n0..n2047) ซึ่งตรงกับ 'Inception v3' "
         "เป็นค่าเริ่มต้นที่เหมาะสมที่สุด แต่ถ้าตอนฝึกโมเดลใน Orange เลือก embedder อื่น "
         "ต้องเปลี่ยนให้ตรงกัน มิฉะนั้นผลทำนายจะผิดพลาด"
)
...
if st.button("ทำนายผล"):
    if model is None:
        st.error("กรุณาเลือกหรืออัปโหลดไฟล์โมเดลก่อนทำนายผล")
